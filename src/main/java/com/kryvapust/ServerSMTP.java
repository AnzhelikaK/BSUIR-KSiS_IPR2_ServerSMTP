    ...
    public void start() {
        ...
        try {
            ...
            while (true) {
                ...
                // чтение сообщения от клиента [EHLO - команда приветствие от клиента]
                readMessage();
                // ответ сервера на команду EHLO - код 250
                sendMessage("250 smtp.localhost is ready");
                // чтение сообщения от клиента [MAIL FROM - команда для указания отправителя сообщения]
                readMessage();
                // ответ сервера на команду MAIL FROM - код 250
                sendMessage("250 OK");
                // чтение сообщения от клиента [RCPT TO - команда для указания получателя сообщения]
                readMessage();
                // ответ сервера на команду RCPT TO - код 250
                sendMessage("250 OK");
                // чтение сообщения от клиента [DATA - команда для указания тела письма]
                readMessage();
                ...
                String line = ""; // переменная, в которую записывается каждая строка сообщения от клиента
                ...
                readMessage();  // чтение сообщения от клиента [QUIT - завершение соединения]
